...
from terrapyn.ows import parsing
...
class GetMapMixin(common.OWSMixinBase):
    ...
    def GetMap(self, r, kwargs):
        ...
        if item and not parms['fresh']:
            return HttpResponse(item, content_type='image/'+parms['format'])
        ...
        resp = HttpResponse(ret, content_type=fmt if '/' in fmt else 'image/'+fmt)
        return resp
...
class GetFeatureInfoMixin(common.OWSMixinBase):
    ...
    def GetFeatureInfo(self, r, kwargs):
        ...
        if parms['callback']:
            ...
            return HttpResponse(etree.tostring(elt, xml_declaration=True), content_type='application/vnd.ogc.wms_xml')
